...
BEGIN
    ...
    FROM hybrid_search hs
WHERE
    hs.keywords IS NOT NULL
        AND EXISTS (
            SELECT 1 
            FROM unnest(hs.keywords) k 
            WHERE lower(k) = lower(search_query)
        )
...
-- Function for text search with pre-filtered documents
CREATE OR REPLACE FUNCTION match_documents_by_text_filtered(
    search_query text,
    filtered_ids text[],
    match_count int DEFAULT 5
)
...
BEGIN
    ...
    FROM hybrid_search hs
    WHERE
        hs.chunk_id = ANY(filtered_ids)  -- Filter by pre-selected IDs
        AND hs.keywords IS NOT NULL      -- Validate keywords are not null
        AND cardinality(hs.keywords) > 0 -- Validate keywords are not empty
        AND (
