...
export function Header({ messages, showTrigger = false, onNewChat }: HeaderProps) {
  const handleLogout = () => {
    window.close();
  };
  ...
  return (
    <div className="flex w-full items-center justify-between px-4 md:px-8">
      <div className="flex items-center gap-1.5">
        {showTrigger && (
          <>
            <SidebarTrigger className="inline-flex items-center justify-center p-2 rounded-md hover:bg-white/10 transition-colors [&_svg]:w-5 [&_svg]:h-5" />
            <button
              onClick={onNewChat}
        ...
        <div className={cn(
          ...
          showTrigger ? "pl-1" : "pl-2"
        )}>
          <h1 className="text-xl font-bold text-white mb-0.5">PestAI</h1>
          <p className="text-xs text-purple-400">Simplify MODFLOW Pest with AI</p>
        </div>
      </div>
      <button
